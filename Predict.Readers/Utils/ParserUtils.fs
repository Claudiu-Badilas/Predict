...
module ParserUtils =
    ...
    let tryGetDouble (value: string option) =
        match value with
        | Some value ->
            let isValid, doubleValue =
                Double.TryParse(value.Replace(",", "."), NumberStyles.Any, CultureInfo.InvariantCulture)
    ...
    let generateReceiptUniqueId
        ...
        let identifier =
            userIdentifier + dateIdentifier + amountIdentifier + providerIdentifier

        identifier.Substring(0, identifier.Length - 1) |> Some


    let generateTransactionUniqueId
        (userId: int)
        (registrationDate: DateTime option)
        (completitonDate: DateTime option)
        (amount: double option)
        (index: int)
        (provider: Provider option)
