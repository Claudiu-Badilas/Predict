...
namespace DataAnalysis.Repository.Repositories {
    public class ReceiptRepo : IReceiptRepo {
        public async Task<IEnumerable<Receipt>> GetReceiptByUserId(int userId) {
            using (var connection = new NpgsqlConnection(NpsqlConnectionString)) {
                ...
                var sql = @"
                    ...
                    FROM platform.receipt
                    WHERE user_id = @userId;";

                return await connection.QueryAsync<Receipt>(sql, new { userId });
            };
        }
        ...
        public async Task<int> StoreReceipts(IEnumerable<Receipt> receipts) {
            await using (var connection = new NpgsqlConnection(NpsqlConnectionString)) {
                var sql = @"
                INSERT INTO platform.receipt
                    (identifier, ""date"", total_price, total_discount, provider_id, currency_id, user_id)
                VALUES (
                    unnest(@identifiers),
                    unnest(@dates),
                    unnest(@total_prices),
                ...
                return await connection.ExecuteScalarAsync<int>(sql,
                    new {
                        ...
                        user_ids = receipts.Select(x => x.UserId).ToList()
                    });
            }
        }
        ...
        public async Task<int> StorePurchasedProducts(IEnumerable<PurchasedProduct> purchasedProducts) {
            await using (var connection = new NpgsqlConnection(NpsqlConnectionString)) {
                var sql = @"
                INSERT INTO platform.purchased_product
